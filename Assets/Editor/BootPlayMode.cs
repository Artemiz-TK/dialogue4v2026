...
namespace Editor
{
    ...
    internal static class BootPlayMode
    {
        ...
        private static string _pendingOriginalPath;
        ...
        private static void HandleEnteredPlayMode()
        {
            ...
            string bootPath = EditorPrefs.GetString(PrefBootPath, string.Empty);
            bool isOriginalBoot = !string.IsNullOrEmpty(bootPath) && string.Equals(originalPath, bootPath, System.StringComparison.OrdinalIgnoreCase);

            if (isOriginalBoot)
            {
                // If the original scene is _Boot, let BootLoader handle the transition
                Debug.Log("BootPlayMode: Original scene is _Boot, skipping additive load and state set. BootLoader will handle.");
                EditorSceneManager.playModeStartScene = null;
                return;
            }

            _pendingOriginalPath = originalPath;
            Debug.Log($"BootPlayMode: EnteredPlayMode. _pendingOriginalPath='{_pendingOriginalPath}'");
            UnityEngine.SceneManagement.SceneManager.sceneLoaded += OnSceneLoadedInPlay;
            try
            {
        ...
        {
            if (string.IsNullOrEmpty(_pendingOriginalPath))
                return;

            string expectedName = Path.GetFileNameWithoutExtension(_pendingOriginalPath);
            if (!string.Equals(scene.path, _pendingOriginalPath, System.StringComparison.OrdinalIgnoreCase) &&
                !string.Equals(scene.name, expectedName, System.StringComparison.OrdinalIgnoreCase))
                return; // not the scene we requested

            try { UnityEngine.SceneManagement.SceneManager.SetActiveScene(scene); } catch (System.Exception ex) { Debug.LogWarning($"BootPlayMode: Failed to set active scene: {ex}"); }
            ...
            Debug.Log("BootPlayMode: scenes after loading original: " + loadedList);
            ...
            for (int i = 0; i < UnityEngine.SceneManagement.SceneManager.sceneCount; ++i)
            {
                ...
                {
                    UnityEngine.SceneManagement.SceneManager.UnloadSceneAsync(sc);
        ...
        private static void TryUnloadBootDeferred()
        {
            ...
            var orig = UnityEngine.SceneManagement.SceneManager.GetSceneByPath(_pendingOriginalPath);
